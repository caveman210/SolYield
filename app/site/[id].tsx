...
export default function SiteDetailScreen() {
  ...
  useEffect(() => {
    ...
    const checkWorkAssigned = async () => {
      const scheduleCount = await getSchedulesCollection()
        .query(Q.where('site_id', site.id))
        .fetchCount();
      const activityCount = await getActivitiesCollection()
        .query(Q.where('site_id', site.id))
        .fetchCount();
  ...
  const handleArchiveVisit = async () => {
    ...
    try {
      ...
      const futureVisits = await getSchedulesCollection()
        .query(
          Q.where('site_id', site!.id),
          Q.where('date', Q.gte(new Date().toISOString().split('T')[0])),
          Q.where('status', 'scheduled'),
          Q.where('archived', false)
        )
        .fetch();
  ...
  const handleArchiveSite = async () => {
    ...
    try {
      ...
      setAlertConfig({
        visible: true,
        title: 'Site Archived',
        message: `${site.name} has been archived.`,
        type: 'success',
      });
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
          ...
          {checkedInSchedule ? (
            <AnimatedTouchableOpacity
              ...
              style={[
                ...
                { backgroundColor: colors.secondary, opacity: checkingOut ? 0.6 : 1 },
              ]}
            >
              {checkingOut ? (
                <ActivityIndicator color={colors.onSecondary} />
              ) : (
                <Text style={[styles.checkInButtonText, { color: colors.onSecondary }]}>
                  Check-out
                </Text>
              )}
            </AnimatedTouchableOpacity>
        ...
        <Animated.View
          ...
          style={styles.visitsSection}
        >
          <Text style={[styles.visitsSectionTitle, { color: colors.onSurface }]}>
            Scheduled Visits
          </Text>
          {siteVisits.length === 0 ? (
            <View
              style={[
                styles.noVisitsContainer,
            ...
            >
              ...
              />
              <Text style={[styles.noVisitsText, { color: colors.onSurfaceVariant }]}>
            ...
            siteVisits.map((visit) => (
              <View
                key={visit.id}
                style={[styles.visitItemCard, { backgroundColor: colors.surfaceContainerHigh }]}
              >
                <View
                ...
                {visit.description && (
                  <Text
                    style={{ color: colors.onSurfaceVariant, fontSize: 13, marginTop: 8 }}
                    numberOfLines={2}
                  >
                    {visit.description}
                  </Text>
