...
export default function ActivitiesScreen() {
  ...
  const renderActivityItem = useCallback(
    ({ item }: { item: Activity }) => (
      <View style={styles.activityItem}>
        <ActivityCard activity={item} context={contextMap[item.id]} onPress={handleActivityPress} />
      </View>
    ),
    [contextMap, handleActivityPress]
  );
  ...
  return (
    <RNAnimated.View
      style={[
        styles.container,
        { paddingTop: insets.top, backgroundColor: animatedColors.background },
      ]}
    >
      <View style={[styles.header, { backgroundColor: colors.background }]}>
        ...
        <StyledText
          style={{ ...M3Typography.title.large, color: colors.onBackground, fontWeight: '600' }}
        >
          Recent Activities
        </StyledText>
        <TouchableOpacity
        ...
        buttons={[
          {
            text: 'Cancel',
            style: 'cancel',
            onPress: () => setUnarchiveConfig({ visible: false, activity: null }),
          },
